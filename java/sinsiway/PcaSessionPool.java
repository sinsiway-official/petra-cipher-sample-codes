...
public class PcaSessionPool extends Object {
	...
	public static sinsiway.PcaSession getSession() throws PcaException
	{
		...
		synchronized (SharedSessionPool) {
			...
			if (SharedFreePool.empty() || (session=(sinsiway.PcaSession)SharedFreePool.pop()) == null) {
	...
	public static void returnSession(sinsiway.PcaSession session)
	{
		synchronized (SharedSessionPool) {
			if (session != null) SharedFreePool.push(session);
		}
	}

	public static sinsiway.PcaSession getSession(String client_ip, String user_id, String client_program) throws PcaException
	{
		String	hash_key = sinsiway.PcaSession.genHashKey(client_ip,user_id,client_program);
	...
	public static void removeSession(String client_ip,String user_id,String client_program)
	{
		String	hash_key = sinsiway.PcaSession.genHashKey(client_ip,user_id,client_program);
		synchronized (SessionPool) {
			sinsiway.PcaSession session = (sinsiway.PcaSession)SessionPool.remove(hash_key);
			if (session != null) {
				SessionLRU.removeElement(session);
				session.closeSession();
			}
	...
	{
		PcaSession.putKeyInfo(key_info,passwd);
	}
